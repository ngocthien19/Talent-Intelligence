...
export const authApi = {
  ...
  resendOtp: async (email) => {
    const response = await authorizedAxiosInstance.post(`${DEV_API_URL}/api/auth/resend-otp`, { email })
    return response.data
  },
  ...
  refreshToken: async () => {
    const response = await authorizedAxiosInstance.post(`${DEV_API_URL}/api/auth/refresh-token`)
    return response.data
  },
