...
const HRChangePassword = ({ isGoogleLogin = false, fetchProfile }) => {
  const { t } = useLanguage()
  ...
  const onSubmit = async (data) => {
    setIsLoading(true)
    try {
      const response = await hrProfileApi.changePassword({
        currentPassword: data.currentPassword,
        newPassword: data.newPassword
      })

      if (response.success) {
        toast.success(t('hr.profile.passwordChangeSuccess') || 'Đổi mật khẩu thành công!')
        await fetchProfile()
        reset()
      }
    } catch (error) {
      toast.error(error?.message || t('hr.profile.passwordChangeFailed') || 'Đổi mật khẩu thất bại')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    return (
      ...
      >
        ...
        <div className="p-6 md:p-8 text-center py-16">
          ...
          >
            {t('hr.profile.googleLoginTitle') || 'Tài khoản Google'}
          </motion.h3>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {t('hr.profile.googleLoginDesc') || 'Bạn đang đăng nhập bằng tài khoản Google. Vui lòng đổi mật khẩu qua Google để bảo mật.'}
          </motion.p>
          <motion.a
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <FaGoogle size={16} />
            {t('hr.profile.goToGoogle') || 'Đến Google Security'}
          </motion.a>
        </div>
      </motion.div>
    )
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-brand-secondary dark:text-white">
              {t('hr.profile.changePassword') || 'Đổi mật khẩu'}
            </h2>
            <p className="text-sm text-brand-text/60 dark:text-gray-400">
              {t('hr.profile.changePasswordDesc') || 'Cập nhật mật khẩu để bảo mật tài khoản của bạn'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-brand-secondary dark:text-white mb-1.5">
              ...
              {t('hr.profile.currentPassword') || 'Mật khẩu hiện tại'} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <input
                {...register('currentPassword')}
                type={showCurrentPassword ? 'text' : 'password'}
                placeholder={t('hr.profile.currentPasswordPlaceholder') || 'Nhập mật khẩu hiện tại...'}
          ...
          <div>
            <label className="block text-sm font-medium text-brand-secondary dark:text-white mb-1.5">
              ...
              {t('hr.profile.newPassword') || 'Mật khẩu mới'} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <input
                {...register('newPassword')}
                type={showNewPassword ? 'text' : 'password'}
                placeholder={t('hr.profile.newPasswordPlaceholder') || 'Nhập mật khẩu mới...'}
                className={`w-full px-4 py-2.5 bg-brand-bg dark:bg-gray-900 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-transparent transition-all duration-200 text-brand-secondary dark:text-white placeholder:text-brand-text/40 pr-12 ${
                  errors.newPassword
                    ? 'border-red-500 focus:ring-red-500/50'
                    : 'border-brand-light dark:border-gray-700'
          ...
          <div>
            <label className="block text-sm font-medium text-brand-secondary dark:text-white mb-1.5">
              <FaLock size={14} className="inline mr-2 text-brand-text/40" />
              {t('hr.profile.confirmPassword') || 'Xác nhận mật khẩu mới'} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <input
                {...register('confirmPassword')}
                type={showConfirmPassword ? 'text' : 'password'}
                placeholder={t('hr.profile.confirmPasswordPlaceholder') || 'Xác nhận mật khẩu mới...'}
                className={`w-full px-4 py-2.5 bg-brand-bg dark:bg-gray-900 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-transparent transition-all duration-200 text-brand-secondary dark:text-white placeholder:text-brand-text/40 pr-12 ${
