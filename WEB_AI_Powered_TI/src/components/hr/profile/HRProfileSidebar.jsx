...
const HRProfileSidebar = ({ user, profile, activeTab, onTabChange, isGoogleLogin = false }) => {
  const { t } = useLanguage()
  ...
  const menuItems = [
    {
      id: 'info',
      icon: FaUser,
      label: t('hr.profile.info') || 'Thông tin cá nhân'
    }
  ]
  ...
  if (!isGoogleLogin) {
    menuItems.push({
      id: 'password',
      icon: FaKey,
      label: t('hr.profile.changePassword') || 'Đổi mật khẩu'
    })
  }
  ...
  menuItems.push({
    id: 'company',
    icon: FaBuilding,
    label: t('hr.profile.company') || 'Thông tin công ty'
  })
  ...
  return (
    ...
    >
      ...
      <div className="p-6 text-center border-b border-brand-light/50 dark:border-gray-700/50">
        ...
        <span className="inline-flex items-center gap-1 mt-2 px-2.5 py-0.5 text-xs font-medium bg-brand-primary/10 text-brand-primary dark:bg-brand-primary/20 dark:text-brand-light rounded-full border border-brand-primary/20 dark:border-brand-primary/30">
          HR
        </span>
        {isGoogleLogin && (
          <motion.span
            ...
            className="inline-flex items-center gap-1 mt-1.5 ml-1.5 px-2.5 py-0.5 text-xs font-medium bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 rounded-full border border-blue-200 dark:border-blue-800"
          >
            <FaGoogle size={10} />
            Google
          </motion.span>
