...
const HRProfileInfo = ({ profile, onUpdateSuccess }) => {
  const { t } = useLanguage()
  ...
  const onSubmit = async (data) => {
    setIsLoading(true)
    try {
      const response = await hrProfileApi.updateProfile(data)
      if (response.success) {
        toast.success(t('hr.profile.updateSuccess') || 'Cập nhật thông tin thành công!')
        await fetchProfile()
        await onUpdateSuccess()
        reset(data)
      }
    } catch (error) {
      toast.error(error?.message || t('hr.profile.updateFailed') || 'Cập nhật thất bại')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      const response = await hrProfileApi.uploadAvatar(formData)
      if (response.success) {
        toast.success(t('hr.profile.avatarUpdateSuccess') || 'Cập nhật avatar thành công!')
        await fetchProfile()
        await onUpdateSuccess()
      }
    } catch (error) {
      toast.error(error?.message || t('hr.profile.avatarUpdateFailed') || 'Cập nhật avatar thất bại')
    } finally {
  ...
  const displayName = profile?.fullname || user?.fullname || 'HR'
      ...
      <div className="p-6 md:p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-bold text-brand-secondary dark:text-white">
            {t('hr.profile.info') || 'Thông tin cá nhân'}
          </h2>
        </div>
        ...
        <div className="flex items-center gap-6 mb-8 pb-6 border-b border-brand-light/50 dark:border-gray-700/50">
          ...
          <div>
            ...
            <p className="text-xs text-brand-text/40 dark:text-gray-500 mt-0.5">
              {t('hr.profile.clickToChangeAvatar') || 'Click vào camera để đổi avatar'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-brand-secondary dark:text-white mb-1.5">
              {t('hr.profile.fullname') || 'Họ tên'} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              ...
              <input
                {...register('fullname')}
                type="text"
                placeholder={t('hr.profile.fullnamePlaceholder') || 'Nhập họ tên...'}
          ...
          <div>
            <label className="block text-sm font-medium text-brand-secondary dark:text-white mb-1.5">
              {t('hr.profile.phone') || 'Số điện thoại'}
            </label>
            <div className="relative">
              ...
              <input
                {...register('phone')}
                type="tel"
                placeholder={t('hr.profile.phonePlaceholder') || 'Nhập số điện thoại...'}
                className={`w-full pl-10 pr-4 py-2.5 bg-brand-bg dark:bg-gray-900 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-transparent transition-all duration-200 text-brand-secondary dark:text-white placeholder:text-brand-text/40 ${
                  errors.phone
                    ? 'border-red-500 focus:ring-red-500/50'
                    : 'border-brand-light dark:border-gray-700'
