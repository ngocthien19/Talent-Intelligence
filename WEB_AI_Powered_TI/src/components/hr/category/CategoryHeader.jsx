    ...
    <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 className="text-2xl font-bold text-brand-secondary dark:text-white">
          {t('hr.category.title') || 'Quản lý danh mục'}
        </h1>
        <p className="text-sm text-brand-text/60 dark:text-gray-400">
          {totalCount > 0 ? (
            <>
              <span>{t('hr.category.showing') || 'Hiển thị'}</span>{' '}
              <span className="font-bold text-brand-primary text-base">
                {totalCount}
              </span>{' '}
              <span>{t('hr.category.categories') || 'danh mục'}</span>
            </>
          ) : (
            <span className="text-red-500 font-medium">
              {t('hr.category.noResults') || 'Không tìm thấy danh mục nào'}
            </span>
          )}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <input
            type="text"
            placeholder={t('hr.category.searchPlaceholder') || 'Tìm kiếm danh mục... (Enter để tìm)'}
            value={searchValue}
        ...
        >
          <FaPlus size={14} />
          <span className="hidden sm:inline">{t('hr.category.add') || 'Thêm danh mục'}</span>
        </button>
      </div>
    </motion.div>
  )
