...
const FIELD_ICONS = {
  candidateId: FaHashtag,
  candidateName: FaUser,
  positionApplied: FaBriefcase,
  position: FaBriefcase,
  overallScore: FaPercentage,
  skillsMatchScore: FaPercentage,
  cultureFitScore: FaPercentage,
  retentionScore: FaPercentage,
  recommendation: FaThumbsUp,
  analysisId: FaHashtag,
  applicationId: FaHashtag,
  email: FaEnvelope,
  sentAt: FaClock,
  jobId: FaHashtag,
  ...
  employmentType: FaBriefcase,
  experienceLevel: FaChartBar,
  requiredSkills: FaTags,
  matchedSkills: FaTags,
  matchCount: FaPercentage,
  totalRequired: FaPercentage,
  status: FaInfoCircle
}
...
const ExtraDataField = ({ fieldKey, value }) => {
  ...
  const Icon = FIELD_ICONS[fieldKey] || FaInfoCircle
  ...
  if (Array.isArray(formattedValue)) {
    ...
    return (
      <div className="flex items-start gap-3 py-2.5">
        <div className="w-8 h-8 rounded-lg bg-brand-primary/10 dark:bg-brand-primary/20 flex items-center justify-center flex-shrink-0 mt-0.5">
              ...
              <AnimatePresence mode="wait">
                {selectedNotification ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center py-16">
                    ...
                    <p className="font-medium text-brand-secondary dark:text-white">
                      {t('hr.selectNotification') || 'Chọn một thông báo để xem'}
                    </p>
                    <p className="text-xs text-brand-text/50 dark:text-gray-500 mt-1">
                      {t('hr.clickNotificationToView') || 'Nhấn vào một thông báo bên trái để xem chi tiết'}
                    </p>
                  </div>
                )}
              </AnimatePresence>
